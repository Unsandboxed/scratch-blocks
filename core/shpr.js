...
Blockly.Shpr.Path.toShapeJS = function(path) {
  var js = 'const UnknownShape = new PathBuilder();\n';
  var cmds = new this(path).toArray(),
    cnt = cmds.length;
  function shiftNum(l, h) {
    var n = Number(cmds.shift());
...
Blockly.Shpr.PathBuilder = function PathBuilder() {
  ...
  this.PATH = null;
  var closed = false;
  var shape = new Blockly.Shpr.Path('m0,0');
  this.cursor = {
    // Moves to the sx and sy coords.
    goto: function(x, y) { return shape.push('M' + x + ',' + y); },
    // Chages the cx and cy by the dx and dy.
    move: function(dx, dy) { return shape.push('m' + dx + ',' + dy); },
  };
  this.line = {
    // Draws to the cx and cy plus dx and dy.
    to: function(dx, dy) { return shape.push('l' + dx + ',' + dy); },
    // Draws like "to" but with dx as 0.
    vertical: function(d) { return shape.push('v' + d); },
    // Draws like "to" but with dy as 0.
    horizontal: function(d) { return shape.push('h' + d); },
    absolute: {
      // Draws from the cx and cy to the sx and sy.
      to: function(x, y) { return shape.push('L' + x + ',' + y); },
      // Draws from the cx and cy to cx and sy.
      vertical: function(y) { return shape.push('V' + y); },
      // Draws from the cx and cy to sx and cy.
      horizontal: function(x) { return shape.push('H' + x); },
    },
  };
  this.curves = {
    quadratic: {
      // Draws from cx and cy to ex and ey and bases on ctx and cty
      absolute: function(ctx, cty, ex, ey) { return shape.push('Q' + ctx + ',' + cty + ' ' + ex + ',' + ey); },
      // Draws from cx and cy to cx and cy plus dx and dy and bases on ctx and cty
      relative: function(ctx, cty, dx, dy) { return shape.push('q' + ctx + ',' + cty + ' ' + dx + ',' + dy); },
      // Continues to cx and cy plus dx and dy.
      continue: function(dx, dy) { return shape.push('t' + dx + ',' + dy); },
      // Continues to the specified ex and ey.
      absoluteContinue: function(ex, ey) { return shape.push('T' + ex + ',' + ey); },
    },
    cubic: {
      // Draws from cx and cy to ex and ey and bases on the points (ctx1, cty1) and (ctx2, cty2)
      absolute: function(ctx1, cty1, ctx2, cty2, ex, ey) { return shape.push('C$' + ctx1 + ',' + cty1 + ' ' + ctx2 + ',' + cty2 + ' ' + ex + ',' + ey); },
      // Draws from cx and cy to cx and cy plus dx and dy and bases on the points (ctx1, cty1) and (ctx2, cty2)
      relative: function(ctx1, cty1, ctx2, cty2, dx, dy) { return shape.push('c$' + ctx1 + ',' + cty1 + ' ' + ctx2 + ',' + cty2 + ' ' + dx + ',' + dy); },
      // Continues to cx and cy plus dx and dy with cd2 using (ctx2, cty2).
      continue: function(ctx2, cty2, dx, dy) { return shape.push('s' + ctx2 + ',' + cty2 + ' ' + dx + ',' + dy); },
      // Continues to the specified ex and ey with cd2 using (ctx2, cty2).
      absoluteContinue: function(ctx2, cty2, ex, ey) { return shape.push('S' + ctx2 + ',' + cty2 + ' ' + ex + ',' + ey); },
    },
    arc: {
      // rx ry rotation large-arc-flag sweep-flag x y
      // https://www.nan.fyi/svg-paths/arcs
      absolute: function(rdx, rdy, rx, largeArc, sweep, ex, ey) { return shape.push('A' + rdx + ' ' + rdy + ' ' + rx + ' ' + Number(largeArc) + ' ' + Number(sweep) + ' ' + ex + ',' + ey); },
      relative: function(rdx, rdy, rx, largeArc, sweep, dx, dy) { return shape.push('a' + rdx + ' ' + rdy + ' ' + rx + ' ' + Number(largeArc) + ' ' + Number(sweep) + ' ' + dx + ',' + dy); },
    },
  };
  // Closing
  this.closePath = function(skipZ, removeM) {
    ...
    if (shape.length > 1) {
      ...
      if (removeM && shape.item(0).at(0).toLowerCase() == 'm') {
        shape.shift();
      }
      if (!removeM && shape.item(1).at(0).toLowerCase() == 'm') {
        shape.shift();
      }
